...
class FavBot
{
			...
			if ($this->getRateLimitStatus()) {
				...
				if ($this->getBlockedUsers()) {
					...
					foreach ($this->aSearchStrings as $iIndex => $sSearchString) {
						...
						//perform search for stuff to fav
	...
	private function doFavs() {

		if (empty($this->aTweets)) {
			$this->logger(3, 'Nothing to fav.');
			$this->halt('Nothing to fav.');
			return FALSE;
		}
		...
		foreach ($this->aTweets as $oTweet) {
			...
			if (!empty($oRet->errors)) {
				$this->logger(2, sprintf('Twitter API call failed: POST favorites/create (%s)', $oRet->errors[0]->message), array('tweet' => $oTweet->id_str));
	...
	//apply all filters to a tweet to determine if we're favoriting it
